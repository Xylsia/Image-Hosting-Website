        ...
        <header>
            ...
            <?php
                if(isset($_GET['searchButton'])){
                    $search = $object->real_escape_string($_GET['search']);
                    $sql = "SELECT picture.path, picture.title, picture.picture_id, AVG(rating.rate) AS average
                            FROM picture INNER JOIN rating ON
                            picture.picture_id = rating.picture_id
                            WHERE picture.title LIKE '%".$search."%'
                            GROUP BY picture.title;";
                    $result = $object->query($sql);
                    $queryResult = $result->num_rows;
                    echo "<h2 class=\"resultSearch\">Search Results: " . $queryResult . "</h2>";
